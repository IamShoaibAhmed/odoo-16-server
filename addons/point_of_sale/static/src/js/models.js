...
class PosGlobalState extends PosModel {
    ...
    async fetchLocationDetails(pickingTypeId) {
        if (!pickingTypeId) return null;
        try {
            // First, fetch the default_location_src_id from stock.picking.type
            const pickingType = await this.env.services.rpc({
            ...
            if (!locationId) {
                console.warn('No default_location_src_id found for picking type:', pickingTypeId);
                return null;
            }

            // Then, fetch the address fields from stock.location
    ...
    async _processData(loadedData) {
        ...
        if (this.picking_type && this.picking_type.id) {
            const location = await this.fetchLocationDetails(this.picking_type.id);
            this.default_location_src_id = location;
        }
    }
...
class Order extends PosModel {
    ...
    export_for_printing(){
        ...
        let outlet_address = this.pos.default_location_src_id || {
            street: '',
            street2: '',
            zip: '',
            city: '',
