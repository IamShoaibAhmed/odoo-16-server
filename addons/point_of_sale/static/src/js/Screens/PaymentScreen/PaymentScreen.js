    ...
    class PaymentScreen extends PosComponent {
        ...
        async validateOrder(isForceValidate) {
            ...
            if (await this._isOrderValid(isForceValidate)) {
                ...
                // 🔻 Calculate Bank Fee before finalizing
                const order = this.env.pos.get_order();
                let bank_fee = 0;

                order.get_paymentlines().forEach(line => {
                    const paymentMethod = line.payment_method;
                    const feeType = paymentMethod.bank_fee_type;
                    const feeValue = paymentMethod.bank_fee_value;

                    if (feeType === 'fixed') {
                        bank_fee += feeValue;
                    } else if (feeType === 'percent') {
                        bank_fee += (line.amount * feeValue) / 100;
                    }
                });

                order.bank_fee = bank_fee;
                // 🔺 End bank fee calculation
