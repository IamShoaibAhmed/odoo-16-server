odoo.define('point_of_sale.ProductScreen', function(require) {
    ...
    class ProductScreen extends ControlButtonsMixin(PosComponent) {
        ...
        async _clickProduct(event) {
            ...
            const total_quantity = (product.warehouse_info || []).reduce(
            ...
            if (total_quantity <= 0) {
                this.showPopup("ErrorPopup", {
                    ...
                    body: this.env._t(`Product "${product.display_name}" is out of stock and cannot be added.`),
                });
                return;
            }
...
// added by shoaib
        async _onClickPay() {
            ...
            const order_data = order.get_orderlines().map(line => ({
                product_id: line.product.id,
                qty: line.quantity,
            }));

            // Perform backend stock check before allowing payment screen
            const insufficient_products = await this.rpc({
                model: 'product.product',
                method: 'check_pos_stock_before_payment',
                args: [order_data, configId],
            });

            if (insufficient_products.length > 0) {
                const lines = insufficient_products.map(p =>
                    `${p.product_name}: Requested ${p.requested_qty}, Available ${p.available_qty}`
                ).join('\n');

                await this.showPopup('ErrorPopup', {
                    title: 'Insufficient Stock',
                    body: `The following items are unavailable in this location:\n\n${lines}`,
                });
                return; // 🚫 Prevent showing payment screen
            }

            // 🔍 Check expiration first
            const expired = await this.rpc({
                model: 'product.product',
                method: 'check_expired_stock_before_payment',
                args: [order_data, configId],
            });

            if (expired.length > 0) {
                const msg = expired.map(e =>
                    `${e.product_name}: Ordered ${e.ordered_qty}, Valid (non-expired) ${e.valid_qty}, Expired ${e.expired_qty}`
                ).join('\n');

                await this.showPopup('ErrorPopup', {
                    title: 'Expired Product Detected',
                    body: `You cannot sell these products:\n\n${msg}`,
                });

                return; // 🚫 Block payment
            }

            // ✅ LOT VALIDATION (existing logic)
            if (
